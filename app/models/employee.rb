class Employee < ApplicationRecord
  ...
  def minor
    if date_of_birth.to_date > 18.years.ago.to_date
      return "Yes"
    else
      return "No"
    end
  ...
  def is_slacker # labels the employee as a slacker if he misses 20% of shifts
    missed_shifts = self.shifts.missed.count
    total_shifts = self.shifts.finished.count
    if total_shifts != 0
      if missed_shifts*100/total_shifts > 20
        return true
      else
        return false
      end 
